...
from pixel import (AutoConfig, AutoModelForTokenClassification, Modality,
                   NERDataset, PangoCairoTextRenderer, PIXELTrainer,
                   PIXELTrainingArguments, PyGameTextRenderer, Split,
                   get_transforms, resize_model_embeddings)
from pixel.data.datasets.ner_dataset import get_labels as get_ner_labels
from pixel.data.datasets.ner_dataset import NERDatasetJIT
from pixel.data.datasets.ner_dataset import \
    write_predictions_to_file as write_ner_predictions_to_file
...
class DataTrainingArguments:
    ...
    hf_dataset_name: Optional[str] = field(
        default=None,
        metadata={"help": "Optional argument to specify dataset name on HuggingFace -- particularly for Naamapadam."}
    )
    hf_dataset_config: Optional[str] = field(
        default=None,
        metadata={"help": "Optional argument to specify language on the specified HuggingFace NER dataset."}
    )
    jit_rendering: bool = field(
            default=False,
            metadata={"help": "Just-in-time rendering to allow working with larger datasets."},
    )
    labels: Optional[str] = field(
        default=None,
        metadata={"help": "Path to a file containing all labels. If not specified, CoNLL-2003 labels are used."},
    )
    use_hf_labels: bool = field(
        default=False,
        metadata={"help": "Use the labels defined within the HuggingFace dataset object."},
    )
...
def log_predictions(
    ...
    eval_dataset: Union[NERDataset, NERDatasetJIT],
    outputs: EvalPrediction,
    prefix: str = "eval",
    labels: List[str] = get_ner_labels("")
):
    ...
    with open(out_file, "w", encoding="utf-8") as f:
        ...
        for ex_id in range(len(eval_dataset)):
            try:
                example = eval_dataset.examples[ex_id]
            except:
                example = eval_dataset.get_ner_example(ex_id)
...
def get_dataset(
    ...
    if data_args.jit_rendering:
        return NERDatasetJIT(
            data_dir=data_args.data_dir,
            use_hf_labels=data_args.use_hf_labels,
            **kwargs,
        )
    else:
        return NERDataset(
            data_dir=data_args.data_dir,
            hf_dataset_name=data_args.hf_dataset_name,
            hf_dataset_config=data_args.hf_dataset_config,
            **kwargs,
        )
...
def main():
    ...
    if training_args.do_eval:
        ...
        if training_args.log_predictions:
            log_predictions(args=training_args, eval_dataset=eval_dataset, outputs=outputs, prefix="eval", labels=labels)
    ...
    if training_args.do_predict:
        ...
        if training_args.log_predictions:
            log_predictions(args=training_args, eval_dataset=test_dataset, outputs=outputs, prefix="test", labels=labels)

        # Save predictions
        # FIXME: find a better way to avoid this.
        try:
            if data_args.hf_dataset_name != "ai4bharat/naamapadam":
