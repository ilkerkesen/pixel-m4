...
class SpanMaskingGenerator:
    ...
    def _mask(
        ...
        num_text_patches: Optional[int] = None
    ):
        delta = 0
        ...
        for attempt in range(100):
            ...
            if span < num_patches_total:
                ...
                if space != 0:
                    ...
                    num_masked_right = mask[left + span : min(left + span + space, num_patches_total)].sum()
    ...
    def __call__(self, num_patches_total: int, num_text_patches: int, num_patches_to_mask: int):
        # Start with an empty mask
        mask = np.zeros(shape=num_patches_total, dtype=np.int32)

        # Greedily try to add mask patches until desired number of masked patches is reached
        mask_count = 0
        while mask_count < num_patches_to_mask:
            max_mask_patches = num_patches_to_mask - mask_count
            max_mask_patches = min(max_mask_patches, self.max_span_length)

            # We attempt to add a span to our mask up to 100 times
            delta = self._mask(mask, max_mask_patches, num_patches_total, num_text_patches)

            if delta == 0:
                # We terminate when no new span could be added to the mask after 100 attempts
                # This can happen before num_patches_to_mask is reached for high masking ratios with
                # strong constraints
                break
            else:
                mask_count += delta
