...
class PIXELPatchEmbeddings(nn.Module):
    ...
    def __init__(self, max_image_size=224, patch_size=16, num_channels=3, embed_dim=768):
        super().__init__()
        max_image_size = to_2tuple(max_image_size)
        patch_size = to_2tuple(patch_size)
        max_num_patches = (max_image_size[1] // patch_size[1]) * (max_image_size[0] // patch_size[0])
    ...
    def forward(self, pixel_values):
        ...
        if height % self.patch_size[0] != 0 or width % self.patch_size[1] != 0:
            raise ValueError(
                f"Input image size ({height}*{width}) is not compatible with patch size {self.patch_size}"
            )
...
class PIXELEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.max_num_patches = self.patch_embeddings.max_num_patches
        # fixed sin-cos embedding
        self.position_embeddings = nn.Parameter(
            torch.zeros(1, self.max_num_patches + 1, config.hidden_size), requires_grad=False
        )
        self.config = config
    ...
    def initialize_weights(self):
        ...
        pos_embed = get_2d_sincos_pos_embed(
            self.position_embeddings.shape[-1], int(self.patch_embeddings.max_num_patches ** 0.5), add_cls_token=True
        )
    ...
    def forward(self, pixel_values, attention_mask=None, patch_mask=None):
        ...
        embeddings = embeddings + self.position_embeddings[:, 1: 1 + embeddings.shape[1], :]

        # logger.debug(f"{self.position_embeddings.shape = }")
...
class PIXELDecoder(nn.Module):
    def __init__(self, config, max_num_patches, dtype):
        super().__init__()
        self.decoder_embed = nn.Linear(config.hidden_size, config.decoder_hidden_size, bias=True)
        self.mask_token = nn.Parameter(torch.zeros(1, 1, config.decoder_hidden_size))
        self.decoder_pos_embed = nn.Parameter(
            torch.zeros(1, max_num_patches + 1, config.decoder_hidden_size), requires_grad=False
        )  # fixed sin-cos embedding
        ...
        self.config = config
        self.dtype = dtype
        self.initialize_weights(max_num_patches)

    def initialize_weights(self, max_num_patches):
        # initialize (and freeze) position embeddings by sin-cos embedding
        decoder_pos_embed = get_2d_sincos_pos_embed(
            self.decoder_pos_embed.shape[-1], int(max_num_patches ** 0.5), add_cls_token=True
        )
    ...
    def forward(
        ...
        # add pos embed
        hidden_states = x + self.decoder_pos_embed[:, : x.shape[1], :]

        # logger.debug(f"{x.shape = }")
        # logger.debug(f"{self.decoder_pos_embed.shape = }")
...
class PIXELForPreTraining(PIXELPreTrainedModel):
    def __init__(self, config):
        ...
        self.decoder = PIXELDecoder(config, max_num_patches=self.vit.embeddings.max_num_patches, dtype=self.vit.dtype)
