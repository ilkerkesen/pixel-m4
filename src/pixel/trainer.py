...
from transformers import (DefaultFlowCallback, ProgressCallback, Trainer,
                          TrainerState, is_torch_tpu_available)
from transformers.debug_utils import DebugOption
from transformers.dependency_versions_check import dep_version_check
from transformers.file_utils import (ModelOutput, is_apex_available,
...
from transformers.trainer_utils import (EvalLoopOutput, EvalPrediction,
                                        HPSearchBackend, PredictionOutput,
                                        ShardedDDPOption, TrainOutput,
                                        denumpify_detensorize, has_length,
                                        speed_metrics)
from transformers.training_args import ParallelMode
from transformers.utils import is_accelerate_available, logging, WEIGHTS_NAME, SAFE_WEIGHTS_NAME, WEIGHTS_INDEX_NAME, is_safetensors_available

from . import MODEL_FOR_CAUSAL_LM_MAPPING_NAMES
...
class PIXELTrainer(Trainer):
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]
                      ) -> Tuple[Optional[torch.Tensor], Optional[ModelOutput]]:
        """
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    with model.no_sync():
                        tr_loss_step, outputs_step = self.training_step(model, inputs)
                else:
                    tr_loss_step, outputs_step = self.training_step(model, inputs)

                if torch.isnan(tr_loss_step) or torch.isinf(tr_loss_step):
                    os.makedirs("nan_inputs", exist_ok=True)
                    logger.info(f"Loss at {self.state.global_step = } is {tr_loss_step}")
                    logger.info(f"Skipping step {self.state.global_step}")
                    output_path = f"nan_inputs/inputs-{self.state.global_step}-{self.args.process_index}-{self.args.local_rank}.pt"
                    logger.info(f"Saving inputs to {output_path}")
                    torch.save(inputs, f"inputs-{self.state.global_step}.pt")
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if (
                ...
                and self.state.global_step % 10000 == 0
